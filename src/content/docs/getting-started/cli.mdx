...
The CLI watches for changes to catalogue and config files:

### Catalogue Files (`.catalogue.tsx`)
...
### Config File (`.uibuilder/config.ts`)

- **`initialState` changed** → page state hot-reloads without restarting the server
- The virtual config module self-accepts HMR updates, so only the affected store updates re-render — no full page reload

<Aside type="tip">
You don't need to restart the server when adding/removing catalogue files or editing `initialState` in the config. The virtual module plugin detects changes and triggers hot updates.
</Aside>
