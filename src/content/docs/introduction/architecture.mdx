...
│  ┌──────────────────────────────────────┐    │
│  │         Zustand Store (immer)         │    │
│  │  nodes, selection, undo/redo, state   │    │
│  └──────────────────────────────────────┘    │
├──────────────────────────────────────────────┤
│           Component Registry                  │
...
- **Page state** — optional state model for [data binding](/editor/data-binding/) (`$path`, `$cond`, `$each`, `$computed`, `$bind`, `$fetch`, `$transform` expressions)
...
|----------|---------|------------|
| **JsonRenderer** | Editor canvas | Custom renderer with DnD Kit integration, selection overlays, resize handles. Resolves all binding expressions for design-time preview. |
| **@json-render/react** | Preview & export | Production renderer, reads the flat JSON spec format. Resolves binding expressions via `JSONUIProvider`. |
