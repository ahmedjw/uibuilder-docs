...
- **Page state** — optional state model for [data binding](/editor/data-binding/) (`$path` expressions)
...
|----------|---------|------------|
| **JsonRenderer** | Editor canvas | Custom renderer with DnD Kit integration, selection overlays, resize handles. Resolves `$path` bindings for design-time preview. |
| **@json-render/react** | Preview & export | Production renderer, reads the flat JSON spec format. Resolves `$path` bindings via `JSONUIProvider`. |
